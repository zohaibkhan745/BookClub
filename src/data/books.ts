...
/**
 * Full book data with all details
 * Used for book detail pages
 */
export const booksData: Record<number, Book> = {
  1: {
    id: 1,
...
/**
 * Helper to convert Book to BookPreview (for list views)
 */
function toPreview(book: Book): BookPreview {
  return {
    ...
    title: book.title,
    author: book.author,
    image: book.image.replace('w=800', 'w=400'), // Use smaller image for lists
  };
}

/**
 * Pre-computed book lists for homepage sections
 */
export const trendingBooks: BookPreview[] = [1, 2, 3, 4, 5, 6].map(id => toPreview(booksData[id]));
export const newArrivals: BookPreview[] = [7, 8, 9, 10, 11, 12].map(id => toPreview(booksData[id]));
export const popularThisWeek: BookPreview[] = [13, 14, 15, 16, 17, 18].map(id => toPreview(booksData[id]));
